...
public class PostgresDAO implements IPostGresDaoService {
	...
	@Override
	public void updateApplicantPage7(Opportunity opportunity) throws PostGressDaoServiceException {
		try{
		...
		List<Applicant> applicants = opportunity.getApplicants();
		Logger.debug("applicants size in DAO " + applicants.size());
		for (Applicant applicant : applicants) {
	...
	public void updateApplicantIncomePage8Or9(Opportunity opportunity)
		...
		Session session = factory.openSession();
		Transaction tx = session.beginTransaction();
		Object o = session.load(Applicant.class, opportunity.getApplicants().get(0).getId());
		Applicant proxyApplicant = (Applicant) o;
		proxyApplicant.setId(opportunity.getApplicants().get(0).getId());
		List<Income> listOfIncome = opportunity.getApplicants().get(0).getIncomes();
		Logger.debug("list Of Income " + listOfIncome.size());
		for (Income income : listOfIncome) {
			proxyApplicant.getIncomes().add(income);
		}
